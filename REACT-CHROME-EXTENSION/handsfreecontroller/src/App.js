...
function App() {
  ...
  useEffect(() => {
    const loadModels = async () => {
      ...
      const weight1 = await faceapi.fetchNetWeights(chrome.runtime.getURL('weights/tiny_face_detector_model.weights'));
      const weight2 = await faceapi.fetchNetWeights(chrome.runtime.getURL('weights/face_landmark_68_model.weights'));
      const weight3 = await faceapi.fetchNetWeights(chrome.runtime.getURL('weights/face_recognition_model.weights'));
      await Promise.all([
        faceapi.nets.tinyFaceDetector.load(weight1),
        faceapi.nets.faceLandmark68Net.load(weight2),
        faceapi.nets.faceRecognitionNet.load(weight3)
